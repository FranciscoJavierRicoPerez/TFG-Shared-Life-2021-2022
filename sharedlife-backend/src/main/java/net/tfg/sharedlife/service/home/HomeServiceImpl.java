...
public class HomeServiceImpl implements HomeService {
    ...
    @Override
    public Home createHome(HomeDTO homeDTO) throws HomeException {
        if (null == homeDTO) {
            throw new HomeException(ErrorMessages.HOME_INFORMATION_ERR);
        }
        // Comprobar si existe una vivienda con los datos de Direccion, Piso, Numero y Ciudad
        // si existe no lanzar una excepción con un mensaje de error.
        if (homeRepository.existsByAddressAndFloorAndCityAndNumber(
                homeDTO.getAddress(), homeDTO.getFloor(), homeDTO.getCity(), homeDTO.getNumber())
        ) {
            throw new HomeException(ErrorMessages.ERR_HOME_ALREADY_EXISTS);
        }
